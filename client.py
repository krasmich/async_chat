...
from utils import receive_message, send_message, create_presence, \
    PORT, HOST, MESSAGE, ACTION, TIME, ACCOUNT_NAME, MESSAGE_TEXT, SENDER, EXIT, DESTINATION, RESPONSE, ERROR
...
class Client:
    ...
    def main(self):
        ...
        self.server_address, self.server_port, self.client_name = arg_parser()
        ...
        try:
            ...
            clientsocket.connect((self.server_address, self.server_port))
            send_message(clientsocket, create_presence(self.client_name))
            answer = self.process_server_answer(receive_message(clientsocket))
            ...
            receiver = threading.Thread(target=self.message_from_server, args=(clientsocket, self.client_name))
            receiver.daemon = True
            receiver.start()

            # затем запускаем отправку сообщений и взаимодействие с пользователем.
            user_interface = threading.Thread(target=self.user_interactive, args=(clientsocket, self.client_name))
...
def main():
    client_name, server_address, server_port = arg_parser()
